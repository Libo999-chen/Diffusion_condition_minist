...
def Euler_Maruyama_sampler(score_model, 
                           ...
                           device='cuda', 
                           eps=1e-3):
  """Generate samples from conditional score-based models with the Euler-Maruyama solver.
  ...
  with torch.no_grad():
    for time_step in tqdm.tqdm(time_steps):      
      ...
      # Pass both `x` and `y` to the score model
...
def pc_sampler(score_model, 
               ...
               device='cuda',
               eps=1e-3):
  """Generate samples from conditional score-based models with Predictor-Corrector method.
  ...
  Args:
    ...
    num_steps: The number of sampling steps. 
      Equivalent to the number of discretized time steps.    
    snr: The signal-to-noise ratio for Langevin MCMC.
    device: 'cuda' for running on GPUs, and 'cpu' for running on CPUs.
    eps: The smallest time step for numerical stability.
